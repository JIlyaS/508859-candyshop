...
(function () {
  ...
  function generateFilterCount(goods) {
    ...
    for (var i = 0; i < goods.length; i++) {
      ...
      if (goods[i].nutritionFacts.sugar === false) {
        countSugarFree += 1;
      }
      if (goods[i].nutritionFacts.vegetarian === true) {
        countVegetarian += 1;
      }
      if (goods[i].nutritionFacts.gluten === false) {
        countGlutenFree += 1;
      }
    }
  ...
  // function removeFilterArray(arr) {
  //   arrFilter.reduce(function (prevElement, currentElement) {
  //     return arr.includes(currentElement) ? prevElement : prevElement.concat(currentElement);
  //   }, []);
  // }
  ...
  function generateFilterPrice(goods) {
    ...
    var min = parseInt(arrayMin(), 10);
    var max = parseInt(arrayMax(), 10);
